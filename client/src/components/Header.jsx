...
export default function Header({ user, selectedPart, onChangePart, onLogout, onReturnToAdmin }) {
    ...
    <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
      <div className="max-w-lg mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold text-gray-900">Formerly</h1>
        </div>

        {/* Right side — part badge + user avatar */}
        <div className="flex items-center gap-2">
          {/* Dashboard link for admins in receipt flow */}
          {onReturnToAdmin && (
            <button
              onClick={onReturnToAdmin}
              className="px-2.5 py-1 bg-gray-100 rounded-full text-xs font-medium text-gray-600 hover:bg-gray-200 transition-colors flex items-center gap-1"
            >
          ...
          {user && (
            <div className="relative">
              <button
                ...
                className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-sm font-semibold text-gray-600 hover:bg-gray-300 transition-colors overflow-hidden"
                title={user.name}
              >
                {user.picture ? (
