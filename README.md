...
│   ├── services/    # Nova AI, Google Sheets, Storage
...
5. **Review + Sheet Preview** → See editable fields + live Google Sheets row preview
6. **Confirm & Submit** → Row is appended to the club's Google Sheet tab

## Team Split

**Team A (Frontend):** Everything in `client/` — signup/login UI, part picker (filtered by user), receipt capture, review card with sheet preview, state management, API calls
**Team B (Backend):** Everything in `server/` — user signup/login endpoints, receipt API routes, Nova integration, Sheets append with SubmittedBy column

## MVP Constraints
- Simple name-based accounts (no passwords — MVP only)
- Users select clubs at signup, part picker filters to their clubs
- No line-item extraction (totals only)
- Fixed sheet columns (with SubmittedBy)
- Append-only to Sheets (no edits)
- Parts are hardcoded: Build, Programming, Outreach
